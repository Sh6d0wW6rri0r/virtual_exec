...
#ifndef VIRTUALEXESERVDEVICE
#define VIRTUALEXESERVDEVICE
...
// ----------------------------------------------------------------
// Classe SERVDEVICE : classe generique de definition d un peripherique distant
// ----------------------------------------------------------------

VECLASS VERemoteDevice {
  protected:
  VERemMControler * linkmem;
  VERemRControler * linkreg;
  VEDevice * linkeddevice;
  int Port;
  NetworkServer * ns;
  ...
  VERemoteDevice();
  void Init();
  void LinkDevice(VEDevice * d);
  void Shutdown();
  void Start();
  void Stop();
  void SetPort(int prt);
  virtual ~VERemoteDevice();
};
